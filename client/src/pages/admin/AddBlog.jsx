    ...
    <form onSubmit={onSubmitHandler} className='flex-1 text-gray-600 h-full overflow-scroll bg-messenger-50/50'>
        <div className='bg-white w-full max-w-3xl p-4 md:p-10 sm:m-10 rounded shadow'>
          ...
          <div className='max-w-lg h-74 pb-16  sm:pb-10 pt-2 relative'>
            ...
            <button type='button' className='absolute bottom-1 right-2 ml-2 text-xs text-white bg-x/70 px-4 py-1.5 rounded hover:underline cursor-pointer' onClick={generateContent}>Generate with AI</button>
          </div>
          ...
          <button type='submit' className='mt-8 w-40 h-10 bg-telegram text-white rounded cursor-pointer text-sm'>Add Blog</button>
        </div>
    </form>
  )
}
